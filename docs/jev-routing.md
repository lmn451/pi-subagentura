...
keywords: [orchestratorv2, jev, typesafe, routing, agents, configuration]
---

# Optional Jev routing

Orchestratorv2 can ask TypeSafe's Jev model to select an existing interactive
child when the requested work is clear but its owner is ambiguous. The parent
lists agents, asks the routing advisor, and sends the task using the existing
`send_interactive_subagent_message` tool. Jev cannot send messages or control
child lifecycle itself.

## Enable

Supply your TypeSafe key through your shell or secret manager, then start Pi:
...
Both the exact router value `jev` and `--orchestratorv2` are required. A key
alone does not enable routing. Restart Pi after changing this environment
...
below to TypeSafe. There is no additional confirmation dialog. Missing keys
and invalid settings produce a safe advisor result without an API request.
...
3. On `kind: "reuse"`, send the original task to the returned `childId` using
...
Requests use native `fetch`, the fixed
...
payload. The API key is sent only in the authorization header.
...
content in tasks submitted to this advisor. Review TypeSafe's
[privacy policy](https://typesafe.ai/privacy) before enabling external processing.
...
Normal tests use mocked transport and do not need a TypeSafe key. They prove
the request, response, failure, and runtime integration contracts; they do not
establish real-world routing accuracy. Provider confidence is a summary of the
probability distribution, not a guarantee that an individual route is correct.
