...
describe("multiplexer-herdr", () => {
  ...
  it("fresh async liveness bypasses the cached pane record", async () => {
    let present = true;
    const calls = installMockExec((call) => {
      if (isCommandProbe(call.file, call.args)) return "";
      if (call.args[0] === "pane" && call.args[1] === "get") {
    ...
    >("../src/multiplexer-herdr");
    const mux = new HerdrMultiplexer();
    const session = "/tmp/other.sock";

    await expect(mux.getPaneLivenessAsync("w1:p2", session)).resolves.toBe(
      "alive",
    );
    present = false;
    await expect(
      mux.getPaneLivenessAsync("w1:p2", session, { fresh: true }),
    ).resolves.toBe("dead");
    expect(
      calls.filter((call) => call.args[0] === "pane" && call.args[1] === "get"),
    ).toHaveLength(2);
  });

  it("fresh async liveness supersedes an older in-flight pane lookup", async () => {
    deferExec = true;
    let present = true;
    const calls = installMockExec((call) => {
      if (isCommandProbe(call.file, call.args)) return "";
    ...
    const older = mux.getPaneLivenessAsync("w1:p2", session);
    expect(pendingExec).toHaveLength(1);
    present = false;
    const fresh = mux.getPaneLivenessAsync("w1:p2", session, {
      fresh: true,
    });
    expect(pendingExec).toHaveLength(2);

    const olderDelivery = pendingExec.shift()!;
    const freshDelivery = pendingExec.shift()!;
    freshDelivery();
