  ...
  it("loads and registers the packed extension in a clean production consumer", () => {
    ...
    const smoke = spawnSync(
      ...
      [
        "--input-type=module",
        "--eval",
        'import { createJiti } from "jiti"; const jiti = createJiti(import.meta.url); const mod = await jiti.import("pi-subagentura", { default: false }); if (typeof mod.default !== "function") process.exit(2); const flags = []; const tools = []; mod.default({ registerTool: (tool) => tools.push(tool.name), registerMessageRenderer() {}, registerFlag: (name) => flags.push(name), registerCommand() {}, registerShortcut() {}, getFlag: () => false, on() {} }); const routing = tools.filter((name) => name.includes("orchestrator")); if (JSON.stringify(routing.sort()) !== JSON.stringify(["list_orchestrator_agents", "update_orchestrator_agent_description"].sort())) process.exit(3); if (!flags.includes("orchestratorv2")) process.exit(4);',
      ],
      { cwd: consumer, encoding: "utf8" },
    );
