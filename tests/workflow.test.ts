...
import { durableWorkflowLiveJobRegistry } from "../src/workflow-jobs";
import { registerSessionScope, removeSessionScope } from "../src/session-scope";
import type { WorkflowProjection } from "../src/workflow-projection-repository";
import type { WorkflowRunStore } from "../src/workflow-run-store";
import type { DurableWorkflowController } from "../src/workflow-durable-plan-runner";
...
describe("background workflow jobs", () => {
  ...
  it("rejects an invalid plan before creating a job or dispatching an agent", () => {
    const runAgent = vi.fn<WorkflowAgentRunner>();
    const before = workflowJobRegistry.size;
    const invalid = {
      ...previewPlan("invalid-plan"),
      phases: [
        {
          id: "phase-a",
          mode: "sequential",
    ...
    expect(() =>
      startWorkflowPlanJob(
        invalid as unknown as Parameters<typeof startWorkflowPlanJob>[0],
        { runAgent },
      ),
    ).toThrow(/1000 tasks/);
    expect(runAgent).not.toHaveBeenCalled();
    expect(workflowJobRegistry.size).toBe(before);
  });

  it("runs a synchronous plan through the shared lifecycle and removes it", async () => {
    const onComplete = vi.fn();
    const job = startWorkflowPlanJob(
      previewPlan("sync-plan"),
      { runAgent: async ({ prompt }) => richOk(`done: ${prompt}`) },
      undefined,
      onComplete,
      undefined,
      "sync",
    ...
    expect(run.result).toEqual([
      { taskId: "task-a", output: "done: first task" },
    ]);
    expect(run.usage).toMatchObject({ input: 11, output: 7, turns: 2 });
    expect(job.snapshot.planState?.status).toBe("done");
    expect(job.snapshot.planState?.tasks["task-a"]).toBe("succeeded");
    expect(onComplete).toHaveBeenCalledWith(job);
    expect(workflowJobRegistry.has(job.id)).toBe(false);
  });

  it("keeps cancelled job and plan snapshots in agreement", async () => {
    const started = Promise.withResolvers<void>();
    const reason = new Error("cancel plan");
    const cancelPlan = previewPlan("cancel-plan");
    cancelPlan.phases[0].tasks.push({
      id: "task-b",
      prompt: "never starts",
      isolation: "in-process",
    });
    const job = startWorkflowPlanJob(cancelPlan, {
      runAgent: ({ signal }) => {
    ...
    const rejection = expect(job.promise).rejects.toBe(reason);

    await started.promise;
    job.abort.abort(reason);
    await rejection;
    expect(job.status).toBe("cancelled");
    expect(job.snapshot.planState?.status).toBe("cancelled");
    expect(job.snapshot.agentsSpawned).toBe(1);
    expect(Object.values(job.snapshot.planState?.tasks ?? {})).not.toContain(
      "running",
    );
    expect(Object.values(job.snapshot.planState?.tasks ?? {})).not.toContain(
      "pending",
  ...
  it("exposes aggregate successful and failing plan usage", async () => {
    const failingPlan = previewPlan("usage-plan");
    failingPlan.phases[0].tasks.push({
      id: "task-b",
      prompt: "second task",
      isolation: "in-process",
    });
    const job = startWorkflowPlanJob(failingPlan, {
      runAgent: async ({ prompt }) =>
        prompt === "first task"
          ? richOk("first done")
          : richFail("second failed"),
    });

    await expect(job.promise).rejects.toThrow("second failed");
    expect(job.status).toBe("error");
    expect(job.snapshot.planState?.status).toBe("error");
    expect(job.snapshot.usage).toMatchObject({
      input: 13,
      output: 8,
      cacheRead: 9,
      cacheWrite: 9,
...
describe("registerWorkflowTool", () => {
  ...
  it("workflow tool has the expected description and parameters", () => {
    ...
    expect(guidance).toContain("stable task IDs");
    ...
    expect(Object.keys(wf.parameters.properties)).toContain("plan");
    expect(Object.keys(wf.parameters.properties)).toContain("durable");
    expect(Object.keys(wf.parameters.properties)).toContain("resumePolicy");
    expect(wf.parameters.properties.resumePolicy.const).toBe("manual");
    expect(wf.parameters.properties.plan.properties.schemaVersion.const).toBe(
      1,
    );
    expect(
      wf.parameters.properties.plan.properties.phases.items.properties.mode
        .const,
    ).toBe("sequential");
    expect(
      wf.parameters.properties.plan.properties.phases.items.properties.tasks
        .items.properties.isolation.const,
    ).toBe("in-process");
    expect(
      wf.parameters.properties.plan.properties.phases.items
        .additionalProperties,
    ).toBe(false);
  });

  it("rejects conflicting inputs and durable requests before job creation", async () => {
    ...
    for (const params of [
      { script, plan: previewPlan() },
      { name: "saved", plan: previewPlan() },
      { script, name: "saved" },
    ]) {
      const result = await execute("", params, undefined, undefined, {});
      expect(result.isError).toBe(true);
  ...
  it("rejects invalid plan input before job creation or child dispatch", async () => {
    ...
    const execute = tools.find((tool) => tool.name === "workflow")!.execute;
    const invalid = {
      ...previewPlan("invalid-tool-plan"),
      phases: [
        {
          id: "phase-a",
          mode: "sequential",
          tasks: [
            { id: "task-a", prompt: "must not dispatch", isolation: "process" },
          ],
        },
      ],
    };
    ...
    const dispatchTrap = new Proxy(
      ...
      {
        get() {
          throw new Error("child dispatch attempted");
        },
      },
    );
  ...
  it("serves async plan status, result, and cancellation through existing tools", async () => {
    const tools: Array<{ name: string; execute: Function }> = [];
    const pi = {
      registerTool: vi.fn((definition: { name: string; execute: Function }) =>
        tools.push(definition),
    ...
    const cancelTool = tools.find((tool) => tool.name === "cancel_workflow")!;

    let releaseResult!: () => void;
    const resultGate = new Promise<void>((resolve) => {
      releaseResult = resolve;
    });
    const completedJob = startWorkflowPlanJob(previewPlan("public-plan"), {
      runAgent: async ({ prompt }) => {
        await resultGate;
        return richOk(`finished: ${prompt}`);
      },
    });

    const running = await statusTool.execute("", {
      workflowId: completedJob.id,
    });
    expect(running.details).toMatchObject({
      ...
      planState: {
        ...
        tasks: { "task-a": "running" },
      },
    });

    releaseResult();
    await completedJob.promise;
    ...
    expect(result.details).toMatchObject({
      status: "done",
      workflowId: completedJob.id,
      usage: { input: 11, output: 7, turns: 2 },
    });
    expect(result.content[0].text).toContain("finished: first task");

    let observedAbort = false;
    const cancelledJob = startWorkflowPlanJob(previewPlan("cancel-plan"), {
      runAgent: ({ signal }) =>
        new Promise<SubagentResult>((_resolve, reject) => {
          signal?.addEventListener(
            ...
            () => {
              observedAbort = true;
              reject(new Error("plan agent aborted"));
            },
            { once: true },
          );
        }),
    ...
    const cancelled = await cancelTool.execute("", {
      workflowId: cancelledJob.id,
    });
    await expect(cancelledJob.promise).rejects.toThrow();
    ...
    workflowJobRegistry.delete(completedJob.id);
    workflowJobRegistry.delete(cancelledJob.id);
  });
