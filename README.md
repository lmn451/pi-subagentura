...
`PI_ORCHESTRATOR_ROUTER=jev` and `--orchestratorv2` are both enabled. Set
`TYPESAFE_API_KEY` through the environment. The parent obtains a candidate
recommendation, then sends the task through its existing messaging tool.
See [Jev routing](docs/jev-routing.md) for setup, external data disclosure, and
failure behavior. Without this opt-in, routing remains unchanged.
