...
child Pi sessions you can watch, attach to, and continue in tmux, Zellij, or
Herdr.
...
tmux, Zellij, or Herdr pane: watch it live, focus it from the current mux, attach from
...
- `mux` — optional backend: `"auto"` (default), `"tmux"`, `"zellij"`, or `"herdr"`. Auto prefers a Herdr-managed pane (`HERDR_ENV=1`), then the attached Zellij/tmux environment, and finally an installed tmux or Zellij detached-session backend. Explicit choice forces that backend. Herdr mode requires Pi to be running inside a Herdr pane and preserves its exact socket path for reload/rehydration.
...
`send_interactive_subagent_message` sends only the follow-up text explicitly requested by its caller. It routes through the persisted backend: tmux and Zellij retain their existing literal text + Enter path, while Herdr uses its semantic [`agent.prompt` socket API](https://herdr.dev/docs/socket-api/) and never falls back to raw pane input. The API method appeared in [Herdr 0.7.5](https://github.com/herdrdev/herdr/releases/tag/v0.7.5), approval/question blocking was added in [0.8.2](https://github.com/herdrdev/herdr/releases/tag/v0.8.2), and 0.9.0 is the minimum for this safe prompt path: it is the first verified release where success waits for queued text and Enter submission to complete. The [Herdr 0.9.0 changelog](https://github.com/herdrdev/herdr/blob/v0.9.0/CHANGELOG.md) verifies this behavior, and CI already pins 0.9.0. Older API versions return an explicit unsupported result while basic Herdr pane operations remain available.

A Herdr `blocked` result can indicate an approval or question UI; the tool reports it for direct inspection and does not read or answer the UI automatically. Transport errors, malformed responses, and timeouts after `agent.prompt` may follow delivery, so they are reported as uncertain and are never retried automatically. Prompt acceptance is not completion: lifecycle and result completion continue to come only from the child's authoritative artifacts. See Herdr's [agent automation guide](https://herdr.dev/docs/agent-automation/).
