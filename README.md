...
Telemetry schema v4 shipped in the 3.6.2 release. The current source adds
schema v5's optional closed `failure_operation` dimension; it contains these
events:
...
| `agent_spawn_failed`         | once per observed rejected spawn attempt; the same agent dimensions, except mux may also be `unknown`, plus required `failure_stage` (`depth_limit`, `capacity`, `context`, `model_resolution`, `session_creation`, `mux_resolution`, `pane_launch`, `state_persistence`, `registration`, `parent_shutdown`, or `unknown`), optional closed `failure_operation` (for example, `interactive_state_write`, `lineage_manifest_write`, or `lineage_bootstrap_write`), and optional rounded `spawn_duration_ms` plus `spawn_duration_bucket` |
